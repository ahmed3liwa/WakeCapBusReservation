...
{
    public class CreateTicketErrorTest
    {
        ...
        private Mock<UserManager<AppUser>> _userManagerMoc;
        ...
        public void Setup()
        {
            ...
            _userManagerMoc = new Mock<UserManager<AppUser>>(store, null, null, null, null, null, null, null, null);
            _userManagerMoc.Object.UserValidators.Add(new UserValidator<AppUser>());
            _userManagerMoc.Object.PasswordValidators.Add(new PasswordValidator<AppUser>());

            _userManagerMoc.Setup(x => x.DeleteAsync(It.IsAny<AppUser>())).ReturnsAsync(IdentityResult.Success);
            _userManagerMoc.Setup(x => x.CreateAsync(It.IsAny<AppUser>(), It.IsAny<string>())).ReturnsAsync(IdentityResult.Success).Callback<AppUser, string>((x, y) => users.Add(x));
            _userManagerMoc.Setup(x => x.UpdateAsync(It.IsAny<AppUser>())).ReturnsAsync(IdentityResult.Success);
            _userManagerMoc.Setup(x => x.FindByEmailAsync(createTicketDto.UserEmail)).ReturnsAsync(new AppUser()
            {
                Email = "ahmedeliwa15@gmail.com"
            }); ;


            _ticketService = new TicketService(_mapperMoc.Object, _unitOfWorkMoc.Object, _userManagerMoc.Object, _ticketRepositoryMoc.Object);
        }
